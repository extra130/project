                        ...
                        <div class="inline-flex flex-wrap gap-2">
                            @foreach($record->tags as $tag)
                                <a href="{{ route('records.index', ['tag' => $tag->name]) }}" 
                                   class="bg-indigo-50 dark:bg-indigo-900/50 text-indigo-600 px-2 py-0.5 rounded text-xs hover:bg-indigo-100 hover:underline">
                                    #{{ $tag->name }}
                                </a>
                            @endforeach
                        </div>
                ...
                <div class="space-y-3">
                    @forelse($record->files->sortBy('sort_order') as $file)
                        <div class="border border-gray-100 rounded p-3 text-sm">
                            ...
                            @if($file->note)
                                <div class="text-xs text-gray-500 dark:text-gray-400 mt-1 italic">{{ $file->note }}</div>
                            @endif
