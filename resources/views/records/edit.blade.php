    ...
    <div class="bg-white dark:bg-gray-800 rounded shadow-sm p-6 max-w-3xl">
        <form method="POST" action="{{ route('records.update', $record) }}">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm">
                        @foreach(['development'=>'開發紀錄','test'=>'測試紀錄','issue'=>'問題紀錄','note'=>'人工備註'] as $val=>$label)
                ...
                {{-- Source --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">來源 *</label>
                    <select name="source" required class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm">
                        <option value="manual" {{ old('source',$record->source)==='manual'?'selected':'' }}>人工</option>
                ...
                {{-- AI Tool --}}
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">使用 AI</label>
                    <input type="text" name="ai_tool" value="{{ old('ai_tool', $record->ai_tool) }}"
                           class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm" placeholder="例如：ChatGPT, Gemini">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">標籤 (Tags)</label>
                    <input type="text" name="tags_input" value="{{ old('tags_input', $tagsString ?? '') }}"
                           class="w-full border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm" placeholder="API, 資安, 資料庫優化 (多個請用逗號分隔)">
                </div>
            </div>
            ...
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('records.show', $record) }}" class="text-gray-500 dark:text-gray-400 text-sm hover:underline">取消</a>
            </div>
        </form>
    </div>
</x-records-layout>
