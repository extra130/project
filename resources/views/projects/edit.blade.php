<x-records-layout :title="'蝺刻摩嚗? . $project->name">
    <x-slot name="header">
        <h2 class="text-xl font-semibold">蝺刻摩撠?</h2>
    </x-slot>
        ...
        <form method="POST" action="{{ route('projects.update', $project) }}">
            ...
            <div class="mb-6">
                ...
                <select name="status" class="border border-gray-300 dark:border-gray-600 rounded px-3 py-2 text-sm">
                    <option value="active" {{ old('status',$project->status)==='active'?'selected':'' }}>?</option>
                    <option value="archived" {{ old('status',$project->status)==='archived'?'selected':'' }}>撠?</option>
                </select>
                <p class="text-xs text-gray-400 mt-1">撠?敺?隞?”?芷嚗??舀閰Ｕ?/p>
            </div>
            ...
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('projects.show', $project) }}" class="text-gray-500 dark:text-gray-400 text-sm hover:underline">??</a>
            </div>
        </form>
    </div>
</x-records-layout>
