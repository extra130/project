<x-records-layout title="銵???>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                <h2 class="text-xl font-semibold">?? 銵???/h2>
            ...
            <div class="flex items-center space-x-4">
                ...
                <span class="text-lg font-bold text-gray-800 dark:text-gray-200 w-24 text-center">{{ $year }} 撟?{{ $month }} ??/span>
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('calendar.index') }}" class="text-sm text-indigo-600 hover:underline">??祆?</a>
                @if(Auth::user()->isEditor())
                    <form action="{{ route('calendar.sync-holidays') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="text-sm bg-indigo-50 dark:bg-indigo-900/50 text-indigo-600 border border-indigo-200 px-3 py-1.5 rounded hover:bg-indigo-100 flex items-center transition-colors" title="敺摨????甇亙?摰???>
    ...
    <div class="bg-white dark:bg-gray-800 rounded shadow-sm overflow-hidden">
        ...
        <div class="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider py-2">
            <div class="text-red-500">?望</div>
            <div>?曹?</div>
            <div>?曹?</div>
            <div>?曹?</div>
            <div>?勗?</div>
            <div>?曹?</div>
            <div class="text-red-500">?勗</div>
        </div>
