<x-records-layout title="行事曆">
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <a href="{{ route('records.index') }}" class="text-sm bg-white border border-gray-300 text-gray-700 px-3 py-1.5 rounded hover:bg-gray-50 flex items-center transition-colors">
                    <span class="mr-1">📋</span> 切換列表模式
                </a>
            </div>
            <div class="flex items-center space-x-4">
                <a href="{{ route('calendar.index', ['year' => $prevMonth->year, 'month' => $prevMonth->month]) }}" class="text-gray-500 hover:text-indigo-600 p-1">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg>
                </a>
                <span class="text-lg font-bold text-gray-800 w-24 text-center">{{ $year }} 年 {{ $month }} 月</span>
                <a href="{{ route('calendar.index', ['year' => $nextMonth->year, 'month' => $nextMonth->month]) }}" class="text-gray-500 hover:text-indigo-600 p-1">
            ...
            <div class="flex items-center space-x-3">
                ...
                @if(Auth::user()->isEditor())
                    <form action="{{ route('calendar.sync-holidays') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="text-sm bg-indigo-50 text-indigo-600 border border-indigo-200 px-3 py-1.5 rounded hover:bg-indigo-100 flex items-center transition-colors" title="從政府公開資料同步國定假日">
    ...
    <div class="bg-white rounded shadow-sm overflow-hidden">
        ...
        <div class="grid grid-cols-7 border-l border-gray-200 auto-rows-fr" style="grid-auto-rows: minmax(120px, auto);">
            @foreach($days as $day)
                <div class="border-b border-r border-gray-200 p-2 flex flex-col 
                    {{ $day['isCurrentMonth'] ? 'bg-white' : 'bg-gray-50' }} 
                    {{ $day['isToday'] ? 'bg-indigo-50 border-indigo-200' : '' }}
                    {{ isset($day['holiday']) ? 'bg-red-50/30' : '' }}">
